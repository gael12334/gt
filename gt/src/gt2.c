...
static void gt_print_generic(gt_generic generic, gt_type type, int fore, int back,
...
int gt_explicit_test_function(const char* name) {
  ...
  if (mode == GT_LOOP_LEAVE) {
    ...
    if (global.Function.failure_nb == 0 && global.Function.success_nb > 0) {
      ...
      gt_println(36, 0, "%lu test case passed, %lu test case failed",
                 global.Function.success_nb, global.Function.failure_nb);
    } else if (global.Function.failure_nb > 0 && global.Function.success_nb == 0) {
...
int gt_explicit_test_case(const char* name) {
  ...
  if (mode == GT_LOOP_LEAVE) {
    ...
    gt_print_indent();
    gt_print(33, 0, "%s ", name);
    for (uint64_t i = 0; i < (30 - length); i++) { gt_print(0, 0, "."); }
    ...
    if (global.Case.result == GT_LOOP_ENTER) {
      ...
      gt_print_indent();
      gt_print_indent();
      gt_println(0, 0, "expr: %s %s %s", global.Assert.A.expression,
                 gt_op[global.Assert.mode], global.Assert.B.expression);
      gt_print_indent();
      gt_print_indent();
...
int gt_explicit_assert(gt_expression a, gt_mode mode, gt_expression b, const char* file,
                       unsigned line) {
