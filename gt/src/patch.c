/*
    ...
    Patch:
        0x90,                                       // nop
        0x48, 0x8b, 0x05, 0xFF, 0xFF, 0xFF, 0xFF,   // mov rax, qword ptr [hijack]    ;
   offset from next instruction to hijack symbol 0x4c, 0x8b, 0x25, 0xed, 0xff, 0xff, 0xff,
   // mov r12, qword ptr 0xedffffff  ; offset from next instruction to endbr64 0x49, 0x3b,
   0xc4,                           // cmp rax, r12 0x0f, 0x84, 0x1a, 0x00, 0x00, 0x00, //
   jz 0x1a                        ; jumps to start of function, after padding 0x55, //
   push rbp 0x48, 0x89, 0xe5,                           // mov rbp, rsp 0x48, 0x8b, 0x1d,
   0xFF, 0xFF, 0xFF, 0xFF,   // mov rbx, qword ptr [hook]      ; offset from next
   instruction to hook symbol 0x48, 0xc7, 0xc0, 0x00, 0x00, 0x00, 0x00,   // mov rax, 0x0
        0xff, 0xd3,                                 // call rbx
        0x5d,                                       // pop rbp
        0xc3                                        // ret
*/
...
static elf_mock_patch_layout elf_mock_patch = {
    ...
    .i1_offset32_hijack =
        {0xff, 0xff, 0xff,
         0xff}, // 4    | offset32 -> offset from IP to target function pointer
    .i2_mov_r12_qwordptr = {0x4c, 0x8b, 0x25}, // 3    | MOV R12, qword ptr [offset32]
    .i2_offset32 =
        {0xed, 0xff, 0xff,
         0xff}, // 4    | offset32 -> offset from IP to first byte of current function
    .i3_cmp_rax_r12 = {0x49, 0x3b, 0xc4}, // 3    | CMP RAX, R12
    .i4_jnz_near = {0x0f, 0x85},          // 2    | JNZ offset32
    .i4_offset32 = {0x1a, 0x00, 0x00,
                    0x00}, // 4    | offset32 -> offset from IP to next instruction
    ...
    .i7_offset32_mock =
        {0xff, 0xff, 0xff,
         0xff}, // 4    | offset32 -> offset from IP to mock function pointer
    .i8_mov_rax_imm32 = {0x48, 0xc7, 0xc0}, // 3    | MOV RAX, immediate32
    .i8_immediate32 = {0x00, 0x00, 0x00,
                       0x00},    // 4    | immediate32 -> literal 32 bits integer value
...
static elf_main_patch_layout elf_main_patch = {
    ...
    .i3_offset32_main =
        {0xff, 0xff, 0xff,
         0xff}, // 4    | 32 bit offset of next instruction to 1st byte of gt_test_main
...
int elf_load_symbol_info(Elf64_Shdr* section, Elf64_Sym* symbol,
                         elf_symbol_info* info_ref) {
  ...
  elf_check(
      elf_get_strtab_shdr_text(info_ref->name_section, symbol->st_name, &info_ref->name));
...
static int elf_load_function_info(Elf64_Shdr* section, Elf64_Sym* symbol,
                                  elf_function_info* info_ref) {
  if (!section || !symbol || !info_ref) {
    return elf_stack_error(ELF_ERR_NULL);
  }
  ...
  while (text[*padding] == NOP && *padding < text_size) { (*padding)++; }

  return elf_stack_error(ELF_OK);
}

static int elf_patch_function(elf_function_info* function, elf_symbol_info* hijack,
                              elf_symbol_info* mock) {
  ...
  offset = function->symbol.offset + function->endbr64 + (uint8_t*)(hijack_ptr + 1) -
           (uint8_t*)&elf_mock_patch;
  *hijack_ptr = hijack->offset - offset;

  offset = function->symbol.offset + function->endbr64 + (uint8_t*)(mock_ptr + 1) -
           (uint8_t*)&elf_mock_patch;
  *mock_ptr = mock->offset - offset;

  elf_check(elf_set_sym_bytes(function->symbol.symbol, function->endbr64,
                              (uint8_t*)&elf_mock_patch, sizeof(elf_mock_patch)));
  return elf_stack_error(ELF_OK);
}
...
static int elf_patch_entrypoint(elf_function_info* entry, elf_function_info* test_entry) {
  ...
  int32_t offset = entry->symbol.offset + entry->endbr64 +
                   (uint8_t*)(test_entry_ptr + 1) - (uint8_t*)patch_start;
  *test_entry_ptr = test_entry->symbol.offset - offset;

  elf_check(elf_set_sym_bytes(entry->symbol.symbol, entry->endbr64,
                              (uint8_t*)&elf_main_patch, sizeof(elf_main_patch)));
  return elf_stack_error(ELF_OK);
}
...
static int elf_patch_elf_impl(const char* entrypoint, int verbose) {
  ...
  while ((error = elf_get_sym_wtype(section, STT_FUNC, &it)) == ELF_OK) {
    ...
    if (strcmp(gtmain.symbol.name, func.symbol.name) == 0) {
      ...
      if (verbose) {
        printf("wrote %zu bytes at offset %zx.\n", sizeof(elf_mock_patch),
               func.symbol.offset + func.endbr64);
      }
      patched++;
    }
    ...
    // size_t elf_hook_nxinst_offset = fn_info.elf_offset + hook_nxinst_offset +
    // fn_info.endbr64_offset;
    //
    // // distance from next instruction to elf offset of hijack and hook symbols
    // uint32_t elf_hijack_func_distance = hijack_symbol_offset -
    // elf_hijack_nxinst_offset; uint32_t elf_hook_func_distance = hook_symbol_offset -
    // elf_hook_nxinst_offset;

    // pointer in patch for distance from next instruction to elf offset of hijack and
    // hook symbols uint8_t* patch_hijack_bytes = &elf_mock_patch[hijack_patch_offset];
    ...
    //
    // // WRITE PATCH
    // elf_check(elf_set_sym_bytes(fn_info.symbol, fn_info.endbr64_offset, elf_mock_patch,
    // sizeof(elf_mock_patch))); elf_check(elf_print_sym_bytes(fn_info.symbol,
    // fn_info.instructions, fn_info.instructions_size));
  }
  ...
  //
  //     size_t elf_main_nxinst_offset = main_info.elf_offset + main_nxinst_offset +
  //     main_info.endbr64_offset; uint32_t elf_main_func_distance =
  //     test_main_info.elf_offset - elf_main_nxinst_offset; uint8_t* patch_main_bytes =
  //     &elf_main_patch[]
  //
  //                                 elf_check(elf_set_sym_bytes(hijack_symbol, 0,
  //                                 (elf_bytebuffer)&main_offset, sizeof(main_offset)));
  //     elf_check(elf_set_sym_bytes(hook_symbol, 0, (elf_bytebuffer)&test_main_offset,
  //     sizeof(test_main_offset)));
  //
